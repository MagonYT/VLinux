...
| Internal SSD | Firmware bring-up | SART mapping/readbacks and two buffer replies observed; IOP ON and DMA unverified; no Linux block device |
...
The corrected buffer candidate passed memory planning and sent two grants, then
held at 78 when endpoint-4 acknowledgments exhausted its transmit cap. The next
candidate extends the bounded capture. Matching VM and native evidence are
tracked separately in [SSD bring-up](SSD.md).
